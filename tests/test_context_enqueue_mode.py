...
def _run_with_mode(notification_mode: str):
  ...
  mock_db = MagicMock()
  with (
    patch("worker.context.DBService") as MockDBService,
    patch("worker.context.TelegramNotification"),
...
def test_both_channels_verbose_when_verbose():
  ...
  with (
    ...
    ctx.notifier.send_message("individual msg")
    assert (
      mock_db.enqueue_notification.call_args.kwargs["mode"]
      == NotificationModeEnum.VERBOSE
    )
