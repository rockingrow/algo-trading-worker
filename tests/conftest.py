...
The SOLID refactor introduced injection seams (an injectable MetaTrader5 gateway,
an ExecutionConfig value object, role-specific persistence/notifier protocols)
that let the whole trading core be exercised on any OS — no native MetaTrader5
package required.

Required ``Settings`` env vars are populated here, before any ``worker`` module
is imported, because ``worker.settings`` instantiates the singleton at import
time.
"""
...
# ── Populate required settings BEFORE importing worker.* ─────────────────── #
...
from worker.core.config import ExecutionConfig  # noqa: E402
from worker.schemas.signal_schema import SignalActionEnum, SignalSchema  # noqa: E402

# ── Value-object / DTO builders ──────────────────────────────────────────── #


def make_symbol_info(**overrides):
  """A broker symbol_info-like object with sensible defaults (XAUUSD-ish)."""
  defaults = {
    "name": "XAUUSDc",
    "point": 0.01,
    "digits": 2,
...
def make_position(ticket=111, type_=0, volume=1.0, price_open=2000.0, tp=2050.0, magic=42, symbol="XAUUSDc", comment=""):
  return SimpleNamespace(
    ticket=ticket, type=type_, volume=volume, price_open=price_open, tp=tp, magic=magic, symbol=symbol, comment=comment
  )
...
# ── Configurable fake MetaTrader5 gateway ────────────────────────────────── #
