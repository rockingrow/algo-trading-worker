...
def symbol_open_rejection(db_service, signal: SignalSchema) -> Optional[str]:
...
def max_open_orders_rejection(db_service, settings: dict, signal: SignalSchema) -> Optional[str]:
  """Return a reason string when *signal* (a LONG/SHORT entry) must be rejected
