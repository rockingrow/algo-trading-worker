...
- **Multiple strategies on one symbol (`FOREX_ALLOW_MULTI_STRATEGY_PER_SYMBOL` / `CRYPTO_ALLOW_MULTI_STRATEGY_PER_SYMBOL`):** By default a `LONG`/`SHORT` entry on a symbol **another strategy already holds** is rejected as a netting conflict (`SignalHandler._handle_entry`, before any cleanup runs — `cancel_all_orders` is symbol-scoped on a CEX, so proceeding would silently wipe the other strategy's SL/TP). Setting the toggle for the active market lets the entry through, so the two strategies hold *parallel* positions on that symbol. The flag is resolved per market by `ExecutionConfig.from_dict` (the FOREX worker never reads the crypto var and vice-versa) and exposed to the handler as `BaseMarketStrategy.allows_multi_strategy_per_symbol`; when it is on, the startup banner reports it.

  - **FOREX** is the market this is actually built for: each strategy trades under its own magic number, so the platform keeps tickets, SL/TP and closes fully isolated (see the magic-based isolation above). It requires a **hedging** account — on a netting account the broker merges both strategies into one position and the isolation is lost.
  - **CRYPTO** has no equivalent: Binance USDⓈ-M nets every strategy on a symbol into one shared position, so enabling it there is an explicit operator override (see also `CryptoExecutor._netting_conflict`).
