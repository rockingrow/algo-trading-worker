...
- TRADE events now carry a `gateway` field (`worker/schemas/position_schema.py` `PositionEvent.gateway`) alongside `market_type` and `account_id`, populated by `PositionCDC` from the processor's gateway value (MT5 / BINANCE). The broker receives all three identity components on every TRADE message, so it can persist and reconstruct the full worker identity `<market>-<gateway>-<account_id>` without parsing it out of any single field.
