...
- **Centralised icon palette via the `emoji` library (`worker/icons.py`).** All 16 Telegram notification emoji (🟢 🔴 🛑 ⚠️ ✅ ❌ 🛡️ 🚨 🚫 🔄 ⚡ ❓ ⚙️ 🖐 🔌 ⏳) are now declared as named constants in a single module using the [`emoji`](https://pypi.org/project/emoji/) library (`emoji>=2.0.0`). All notification modules (`crypto/message_presenter`, `forex/message_presenter`, `mt5_event_job`, `nats_service`, `forex/signal_processor`) import from `worker.icons` instead of embedding raw Unicode codepoints — the full icon set lives in one place.
...
- **External (manual) close ingestion on CRYPTO.** `parse_order_trade_update` now also surfaces a reduce-only `MARKET` fill placed *outside* the worker — a discretionary close from the Binance web UI / mobile / a third-party API client — as an `ExchangeCloseReason.MANUAL` event, alongside the existing SL/TP/liquidation reasons. To tell these apart from the worker's *own* reduce-only closes (an otherwise identical fill), every worker-placed close now stamps its `clientOrderId` with a `WORKER_ORDER_PREFIX` (`awkr_`) marker, declared once in the exchange-agnostic `crypto/base.py` so the executor and the event stream share one source of truth; a fill carrying the marker is skipped (already handled by the signal flow), and a Binance system auto-exit prefix (`autoclose-` / `adl_autoclose` / `settlement_autoclose`) is classified `LIQUIDATION` rather than `MANUAL`.
- **`TP1_MOVE_SL_TO_BREAKEVEN` setting** (default `true`). When `false`, a `TP1` signal only partial-closes and leaves the original entry SL in place, so the runner keeps its initial protection instead of being moved to breakeven. Threaded through `ExecutionConfig` and honoured by `ExecutorBackedMarket.handle_tp1` (which then skips the breakeven move — and its emergency-close fallback — entirely).
- **Account ID in the CRYPTO notification footer.** `get_account_footer(account_id=…)` now renders an `Account ID` line, so crypto management alerts identify the account the same way the FOREX footer does.

### Fixed

- **0.0 fill price on Binance `MARKET` orders.** Binance Futures testnet (and occasionally live) returns `avgPrice="0"` on a fully-filled `MARKET` order, which corrupted the recorded fill price for entries *and* closes. Two complementary fixes: `BinanceFuturesGateway._order_result` now derives the average from `cumQuote / executedQty` when `avgPrice` is 0, and `BaseSignalProcessor._process_message` — the single funnel for every signal action — falls back to the signal's `price` when a successful result still reports a falsy price (the close paths have no access to the signal, so the fallback belongs here). Note `result.get("price", signal.price)` would *not* help: the key is present but zero, not missing.
- **Orphaned DB row blocked a new entry after an external close.** On a new `LONG`/`SHORT`, the entry handler now reconciles *every* active (`OPENED`/`TP1`) DB row for the `(strategy, symbol)` pair to `FORCED_CLOSED` — independently of whether the broker still reported a live position. Previously the DB cleanup ran only when the broker still had a live position, so a position closed externally (exchange SL/liquidation, manual close, or a missed close event) left an orphaned `OPENED` row that then collided with the one-active-per-`(strategy, symbol)` unique index on insert, leaving the new trade live but untracked. The cleanup comment distinguishes the two cases (`Force-closed by new entry signal` vs `Orphaned DB row cleared by new entry (not live on broker)`).
- **Symbol resolution drops an exchange prefix.** `CryptoExecutor.get_symbol` now strips a leading `EXCHANGE:` segment (`BINANCE:BTCUSDT.P`, `OANDA:XAUUSD` → last `:`-segment) before stripping the `.P` perpetual suffix and normalizing a `USD` quote to the configured quote asset, so prefixed upstream symbols resolve correctly.
...
- **Renamed `worker/nats.py` → `worker/nats_client.py`** (the module exporting `NatsClient`). Avoids shadowing the third-party `nats` package it imports; `worker/services/nats_service.py` updated accordingly.
- **Corrected default settings.** `capital_currency` `USC` → `USD`, `risk_percentage` `3.0` → `2.0`, `slippage_deviation` `20` → `100`, matching `.env.example`. These are field defaults only (any `.env` / environment value still wins).
