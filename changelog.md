...
- CRYPTO: the crypto worker's `[Connected]` startup banner now includes a `HEDGE_MODE` line (`Hedge` / `One-way`, from `BINANCE_HEDGE_MODE`), so operators can confirm at a glance which Position Mode the connected worker is configured for alongside `EXCHANGE` and `TESTNET`.
