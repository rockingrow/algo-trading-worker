...
- **FOREX manual-order adoption** — hand-opened MT5 positions (opened from the desktop / mobile / web terminal, carrying a magic this worker never assigned — typically `0`) can now be adopted into the worker's tracking so they complete the normal position cycle (`TP1` / `TP2` / `SL` / `R_SL` / `FLAT`) exactly like a worker-opened trade. Previously such a position was invisible to the magic-scoped signal pipeline: no DB row was ever inserted for it, and every exit signal bailed at the "no tracked position" guard, so the position could never be closed by a signal.
  - `ManualOrderSyncJob` (`worker/jobs/manual_order_sync_job.py`) — a daemon poller (default every `MANUAL_ORDER_SYNC_INTERVAL` = 5s) that runs in the FOREX child process alongside the health thread and `MT5EventJob`. Each scan it asks the executor for live positions whose magic is **not** owned (`ForexExecutor.get_manual_open_positions()`); any that has no DB row yet is inserted as `OPENED` under the reserved `MANUAL_ORDER_STRATEGY`, with an audit-log row written `author='manual'`, and an operator notification is sent. From then on `PositionCDC` publishes the row and exit signals addressed to the reserved strategy close it. The reserved strategy is a single logical slot, so it inherits the one-active-per-`(strategy, symbol)` DB invariant: a second live manual position on the same symbol is left untracked and logged rather than colliding with the unique index. A malformed position never aborts the scan (per-position try/except), and re-inserting an already-tracked ticket is skipped (idempotent across scans and worker restarts).
  - `MANUAL_ORDER_SYNC_ENABLED` (bool, default `false`) env var — master switch for the whole feature. When `false` (the default) nothing changes: the job never starts and the executor's manual-position matching stays completely inert.
  - `MANUAL_ORDER_STRATEGY` (str, default `MANUAL`) env var — the reserved strategy name that owns adopted manual orders. It must be **disjoint** from `STRATEGY_MAGIC_MAP`. Upstream must address the manual trade's exit signals (`TP1`/`TP2`/`SL`/`R_SL`/`FLAT`) with this same strategy so the DB lookup and close resolve to the adopted position.
  - `MANUAL_ORDER_SYNC_INTERVAL` (int, default `5`) env var — seconds between terminal scans.
  - `ForexExecutor.get_manual_open_positions(symbol=None)` — live positions whose magic this worker does not own. `get_open_positions` / `get_all_open_positions` now route the reserved `MANUAL_ORDER_STRATEGY` to this "unowned" bucket instead of resolving a magic through `STRATEGY_MAGIC_MAP`, so every exit path (`handle_tp1`, `handle_full_close`, `close_all_positions`, TP1 breakeven SL move) finds and acts on the manual position by strategy — no other exit-path code changed.
  - `BasePlatformGateway.base_symbol()` / `SymbolResolver.base_for()` — best-effort inverse of symbol resolution (e.g. `XAUUSDc` → `XAUUSD`) via the resolver cache, so an adopted position is stored under the **base** symbol upstream exit signals address it by rather than the broker's suffixed tradeable name. Brokers without a suffix are unaffected (identity). The base-class default is identity, so CRYPTO and any non-suffixed platform need no change.
  - `LogAuthorEnum.MANUAL` (`"manual"`) — new position-log author, distinguishing hand-opened adopted rows from `broker` (NATS-signal) and `terminal` (server-closed) rows.
