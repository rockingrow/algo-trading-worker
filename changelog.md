...
- FOREX: the MT5 health thread is now **weekend-aware**, so a routine weekend disconnect no longer floods the logs and Telegram. The FOREX market is closed roughly Fri 22:00 → Sun 22:00 UTC while the broker's trade server is offline for weekly maintenance — a disconnect in that window is expected, not a crash. On Saturday/Sunday (evaluated in market-local time, `MARKET_TZ_OFFSET_HOURS`, default UTC+7) the loop backs off from the 15s cadence to `MT5_HEALTH_INTERVAL_WEEKEND` (default 15 min), skips the weekday kill-and-relaunch storm (`taskkill terminal64.exe` + up to two rounds of 15 reconnect attempts), makes a single lightweight `reconnect(max_attempts=1)` per check so the session re-establishes promptly once the server returns, and sends the "Market Closed" notice only **once** per weekend-disconnect (rearmed when the connection recovers). Weekday behaviour is unchanged — a genuine disconnect still triggers the full aggressive relaunch/reconnect path immediately. The health-loop wait is now interruptible (`stop_event.wait`) so shutdown isn't delayed by the long weekend interval. New module constants `MT5_HEALTH_INTERVAL_WEEKEND` and `MARKET_TZ_OFFSET_HOURS`, plus `is_market_weekend()`, in `worker.settings`.
